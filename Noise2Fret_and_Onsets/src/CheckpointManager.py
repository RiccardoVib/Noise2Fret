...
class DiffusionCheckpointManager:
    """
    ...
    def load_checkpoint(self, filepath, diffusion, optimizer=None, optimizer_count_head=None, optimizer_onset_head=None,
                        scheduler=None, scheduler_count_head=None, scheduler_onset_head=None, device='cpu'):
        """
        Load a checkpoint.

        Args:
            filepath (str or Path): Path to the checkpoint file
            diffusion (nn.Module): diffusion to load weights into
        ...
        if 'model_state_dict' in checkpoint:
            ...
            diffusion.embeddings.load_state_dict(checkpoint['embedding_state_dict'])
            diffusion.count_head.load_state_dict(checkpoint['count_head_state_dict'])
            diffusion.onset_head.load_state_dict(checkpoint['onset_head_state_dict'])
        ...
        if optimizer_count_head is not None and 'optimizer_count_head_state_dict' in checkpoint:
            optimizer_count_head.load_state_dict(checkpoint['optimizer_count_head_state_dict'])

        # Load scheduler state if provided
        if scheduler_count_head is not None and 'scheduler_count_head_state_dict' in checkpoint:
            scheduler_count_head.load_state_dict(checkpoint['scheduler_count_head_state_dict'])

        if optimizer_onset_head is not None and 'optimizer_onset_head_state_dict' in checkpoint:
            optimizer_onset_head.load_state_dict(checkpoint['optimizer_onset_head_state_dict'])

            # Load scheduler state if provided
        if scheduler_onset_head is not None and 'scheduler_onset_head_state_dict' in checkpoint:
            scheduler_onset_head.load_state_dict(checkpoint['scheduler_onset_head_state_dict'])

        return checkpoint

    def load_last_checkpoint(self, model, optimizer=None, optimizer_count_head=None, optimizer_onset_head=None,
                        scheduler=None, scheduler_count_head=None, scheduler_onset_head=None, device='cpu'):
        """
        ...
        """
        return self.load_checkpoint(self.last_path, model, optimizer, optimizer_count_head, optimizer_onset_head, scheduler, scheduler_count_head, scheduler_onset_head, device)

    def load_best_checkpoint(self, model, optimizer=None, optimizer_count_head=None, optimizer_onset_head=None,
                        scheduler=None, scheduler_count_head=None, scheduler_onset_head=None, device='cpu'):
        """
        ...
        Returns:
            dict: Checkpoint contents or None if no checkpoint found
        """
        return self.load_checkpoint(self.best_path, model, optimizer, optimizer_count_head, optimizer_onset_head, scheduler, scheduler_count_head, scheduler_onset_head, device)
...
class CheckpointManager:
    ...
    def load_checkpoint(self, filepath, model, optimizer=None, scheduler=None, device='cpu'):
        """
        Load a checkpoint.
